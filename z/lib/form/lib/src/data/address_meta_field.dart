...
class AddressMetaFieldData extends MetaFormFieldData<Address> {
  ...
  static Future<List<StreamableFormFieldData>> _fieldsData(
    ...
    bool? isRequired,
      ...
      Future.value([
        _address1(initialValue?.address1 ?? '', isRequired ?? false),
        _address2(initialValue?.address2 ?? ''),
        _city(initialValue?.city ?? '', isRequired ?? false),
        _state(initialValue?.stateProvinceRegion ?? ''),
        _postalCode(initialValue?.postalCode ?? '', isRequired ?? false),
      ]);
  ...
  AddressMetaFieldData({
    ...
    bool? isVisible,
          ...
          title: title ?? 'Address',
          initialValue: initialValue,
          fieldsData: _fieldsData(initialValue ?? Address(), isRequired ?? false),
